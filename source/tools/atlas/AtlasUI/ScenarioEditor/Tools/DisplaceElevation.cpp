...
class DisplaceElevation : public StateDrivenTool<DisplaceElevation>
{
	DECLARE_DYNAMIC_CLASS(DisplaceElevation);
...
public:
	...
	{
		bool OnMouse(DisplaceElevation* obj, wxMouseEvent& evt)
		{
			if (evt.LeftDown())
			{
	...
	{
		void OnEnter(DisplaceElevation* obj)
		{
			POST_MESSAGE(BrushPreview, (true, obj->m_Pos));
		}

		void OnLeave(DisplaceElevation*)
		{
		...
		void OnTick(DisplaceElevation* obj, float dt)
		{
			POST_COMMAND(DisplaceElevation, (obj->m_Pos, dt*g_Brush_Elevation.STRENGTH_MULTIPLIER*GetDirection()*g_Brush_Elevation.GetStrength()));
...
IMPLEMENT_DYNAMIC_CLASS(DisplaceElevation, StateDrivenTool<DisplaceElevation>);
